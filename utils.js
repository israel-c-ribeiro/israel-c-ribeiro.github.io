/**
 * @file utils.js
 * @description Script principal com DADOS EMBUTIDOS para evitar erro de carregamento local.
 * @version 16.0 (Fix Definitivo Dr. Israel)
 */

// =================================================================================
// DADOS MANUAIS (HARDCODED) - Edite aqui se precisar mudar números ou textos
// =================================================================================
const MANUAL_DATA = {
    scholarData: {
        profile: {
        ...
        articles: [
            ...
            {
                title: "Unveiling the Impact of Organic Cation Passivation on Structural and Optoelectronic Properties of Two-Dimensional Perovskites",
                year: "2024",
...
// Disponibiliza os dados globalmente IMEDIATAMENTE
...
// =================================================================================
// Módulo: Scholar (Carrega os dados MANUAIS)
// =================================================================================
const scholarScript = (function() {
    ...
    function renderPublications() {
        ...
        if (articlesToShow.length === 0) {
            UI.pubsGrid.innerHTML = `<div class="card"><p>${trans.no_pubs_found || 'Nenhuma publicação encontrada.'}</p></div>`;
        } else {
    ...
    init() {
        document.querySelectorAll('[data-cv-type]').forEach(btn => {
            btn.addEventListener('click', (e) => {
                e.preventDefault();
                alert("Função de geração de PDF simplificada ativa. Para o PDF completo, por favor, verifique a versão online ou entre em contato.");
            });
        });
    }
};
...
const LanguageManager = {
    currentLang: 'pt',
    init() {
        // Tenta carregar traduções, mas segue mesmo se falhar
        fetch('translations.json')
            .then(r => r.json())
            .then(data => { window.translations = data; this.applyLang(); })
            .catch(() => console.warn("Traduções não carregadas, usando fallback visual."));
